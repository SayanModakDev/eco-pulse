...
- **Testing**: 64 backend unit tests (using Node's native `node:test`) covering Zod schema boundaries, LLM output quality, projection math, and API contract compliance. Frontend includes comprehensive React component regression tests (e.g., verifying real-time mitigation plan updates to prevent state regressions).
