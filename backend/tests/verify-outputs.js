...
const scenarios = [
  ...
  },
  {
    ...
    input: 'Took a taxi 10km, ate 2 chicken meals, used 6kwh electricity, and threw 1kg trash',
...
function assertActivityShape(activity, label) {
  ...
  assert.ok(
    ['transport', 'food', 'energy', 'waste', 'other'].includes(activity.category),
    `${label}: invalid category "${activity.category}"`
  );
  assert.equal(typeof activity.value, 'number', `${label}: value must be number`);
  assert.ok(activity.value >= 0, `${label}: value must be non-negative`);
  assert.ok(typeof activity.unit === 'string' && activity.unit.length > 0, `${label}: unit must be non-empty string`);
  assert.ok(typeof activity.description === 'string' && activity.description.length > 0, `${label}: description must be non-empty string`);
  assert.equal(typeof activity.co2eKg, 'number', `${label}: co2eKg must be number`);
  assert.ok(activity.co2eKg >= 0, `${label}: co2eKg must be non-negative`);
  assert.equal(typeof activity.emissionFactorUsed, 'number', `${label}: emissionFactorUsed must be number`);
  assert.ok(activity.emissionFactorUsed >= 0, `${label}: emissionFactorUsed must be non-negative`);
}

function assertSummaryShape(summary, label) {
  assert.equal(typeof summary.totalCo2eKg, 'number', `${label}: totalCo2eKg must be number`);
  assert.ok(summary.totalCo2eKg >= 0, `${label}: totalCo2eKg must be non-negative`);
  assert.equal(typeof summary.dailyBaselineKg, 'number', `${label}: dailyBaselineKg must be number`);
  assert.ok(summary.dailyBaselineKg > 0, `${label}: dailyBaselineKg must be positive`);
  assert.equal(typeof summary.differenceKg, 'number', `${label}: differenceKg must be number`);
  assert.equal(typeof summary.percentageDifference, 'number', `${label}: percentageDifference must be number`);
  assert.ok(
  ...
  if (summary.hotspot) {
    ...
    assert.equal(typeof summary.hotspot.co2eKg, 'number', `${label}: hotspot co2eKg must be number`);
  }
}

function assertChallengeShape(challenge, index, label) {
  const tag = `${label} challenge[${index}]`;
  assert.ok(typeof challenge.title === 'string' && challenge.title.length > 0, `${tag}: title must be non-empty string`);
  assert.ok(typeof challenge.description === 'string' && challenge.description.length > 0, `${tag}: description must be non-empty`);
  assert.equal(typeof challenge.potentialSavingKg, 'number', `${tag}: potentialSavingKg must be number`);
  assert.ok(challenge.potentialSavingKg > 0, `${tag}: potentialSavingKg must be positive`);
  assert.ok(
    ['easy', 'medium', 'hard'].includes(challenge.difficulty),
    `${tag}: invalid difficulty "${challenge.difficulty}"`
  );
  assert.ok(
    ['transport', 'food', 'energy', 'waste', 'other'].includes(challenge.category),
  ...
  for (const scenario of scenarios) {
    it(`[Schema] ${scenario.name}`, async () => {
      const result = await orchestrateCarbonTracking({
        ...
        profileContext: { userId: 'verify', email: 'v@v.com', dailyBaselineKg: 15.0 },
      });

      const label = scenario.name;

      // Top-level fields
      assert.ok(typeof result.rawInput === 'string', `${label}: rawInput must be string`);
      assert.equal(result.rawInput, scenario.input, `${label}: rawInput must match input`);
      assert.ok(result.profileApplied, `${label}: profileApplied must exist`);
      assert.equal(result.profileApplied.userId, 'verify', `${label}: userId must match`);

      // Activities array
      assert.ok(Array.isArray(result.activities), `${label}: activities must be array`);
      assert.ok(result.activities.length > 0, `${label}: activities must not be empty`);
      ...
      // microChallenges
      assert.ok(Array.isArray(result.microChallenges), `${label}: microChallenges must be array`);
      assert.ok(result.microChallenges.length > 0, `${label}: microChallenges must not be empty`);
      for (let i = 0; i < result.microChallenges.length; i++) {
        assertChallengeShape(result.microChallenges[i], i, label);
      }

      // summaryInsight
      assert.ok(typeof result.summaryInsight === 'string', `${label}: summaryInsight must be string`);
      assert.ok(result.summaryInsight.length > 10, `${label}: summaryInsight must be a meaningful sentence`);

      // Verify expected categories were detected
      if (scenario.expectedCategories) {
        const detectedCategories = new Set(result.activities.map(a => a.category));
        for (const expected of scenario.expectedCategories) {
          assert.ok(detectedCategories.has(expected), `${label}: expected category "${expected}" not detected`);
        }
      }
...
describe('Challenge Quality Assessment', () => {
  it('food hotspot → challenges should target food reduction', async () => {
    ...
    for (const ch of result.microChallenges) {
      assert.equal(ch.category, 'food', 'All challenges for food hotspot should be food category');
      assert.ok(ch.potentialSavingKg >= 0.1, 'Saving should be at least 0.1 kg');
      assert.ok(ch.title.length >= 4, 'Title should be at least 4 characters');
      assert.ok(ch.description.length >= 20, 'Description should be at least 20 characters');
    }
  });
  ...
  it('transport hotspot → challenges should target transport reduction', async () => {
    ...
    for (const ch of result.microChallenges) {
      assert.equal(ch.category, 'transport', 'All challenges for transport hotspot should be transport category');
      assert.ok(ch.potentialSavingKg >= 0.1, 'Saving should be at least 0.1 kg');
    }
  });
  ...
  it('energy hotspot → challenges should target energy reduction', async () => {
    ...
    for (const ch of result.microChallenges) {
      assert.equal(ch.category, 'energy', 'All challenges for energy hotspot should be energy category');
    }
  });
  ...
  it('challenges should have distinct titles (no duplicates)', async () => {
    ...
    assert.equal(titles.length, uniqueTitles.size, 'Challenge titles must be unique');
  });
  ...
  it('potentialSavingKg values should be realistic (< 50 kg per challenge)', async () => {
    ...
    for (const ch of result.microChallenges) {
      ...
      assert.ok(ch.potentialSavingKg < 50, `Saving ${ch.potentialSavingKg} kg is unrealistically high`);
    }
  });

  it('summaryInsight should be coherent and related to the hotspot', async () => {
    const testCases = [
      { input: 'I ate 5 beef steaks', hotspot: 'food', keywords: ['food', 'plate', 'swap', 'difference'] },
      { input: 'Drove 100km in my car', hotspot: 'transport', keywords: ['transport', 'car-free', 'trip', 'impact'] },
      { input: 'Used 30kwh electricity', hotspot: 'energy', keywords: ['energy', 'home', 'footprint', 'changes'] },
    ];

    for (const tc of testCases) {
      const result = await orchestrateCarbonTracking({ activityString: tc.input });
